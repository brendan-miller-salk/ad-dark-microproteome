...
#   - Build per–cell-type volcano plots (Noncanonical only) colored by log10(PSM).
...
dark_proteome <- read.csv("../data/microprotein_master.csv") %>%
  dplyr::filter(protein_class_length == "Microprotein") %>%
  dplyr::filter(
    MS_unique_evidence == "True" |
      (MS_unique_evidence == "False" &
         RiboCode == "True" &
         shortstop_label != "PRISM" &
         smorf_type != "isoORF")
  ) %>%
  dplyr::mutate(
    smorf_type = ifelse(Database == "Swiss-Prot", paste0(smorf_type, "SwissProt"), smorf_type),
    smorf_type = ifelse(Database == "TrEMBL",     paste0(smorf_type, "TrEMBL"),     smorf_type),
    Database   = ifelse(Database == "Salk", "Noncanonical", Database),
    Database   = ifelse(Database == "TrEMBL", "Noncanonical", Database)
  ) %>%
  dplyr::filter(
    (Database == "Swiss-Prot" & MS_razor_evidence == "True") |
      (Database == "Noncanonical"     & MS_unique_evidence == "True")
  ) %>%
  dplyr::filter(!duplicated(sequence))
...
plot_data <- gpath_annotated %>%
  ...
  filter(Database == "Noncanonical" | Database == "Swiss-Prot") %>%
  mutate(
    cell_type_general = trimws(cell_type_general),
    celltype          = trimws(celltype)
  )

# Write main summary output
write.csv(plot_data, file.path(OUT_DIR, 'scRNA_Enrichment_summary.csv'), row.names = FALSE)
...
heatmap_data <- plot_data %>%
  ...
  summarise(
    ...
    total_unique_spectral_counts = mean(total_unique_spectral_counts, na.rm = TRUE),
    .groups = "drop"
  )
...
# ------------------------------------------------------------------------------
# Volcano plots per cell_type_general (Noncanonical only)
# ------------------------------------------------------------------------------
...
for (ct in ordered_cell_types) {
  ...
  df_ct <- gpath_annotated %>%
    ...
    arrange(factor(Database, levels = c("Noncanonical", "Swiss-Prot"))) %>%
    filter(Database == "Noncanonical")
